...
public final class APGIntentActionList {
    ...
    private var actionDictionary: [String: APGIntentAction] = [:]
    ...
    public func addAction(token: String,
                          action: @escaping APGIntentActionClosure,
    ...
    public func appearance(token: String, param: String) -> (Bool, Bool?, String?) {
    ...
    public func find(token: String) -> APGIntentAction? {
        actionDictionary[token]
    }

    // MARK: - Execution

    /// Perform the action associated withparam the given token.
    /// - Returns: true if an action was found and executed.
    @discardableResult
    public func perform(token: String, param: String) async -> Bool {
